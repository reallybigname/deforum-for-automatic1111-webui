...
class TestParseqAnimKeys(unittest.TestCase):
    ...
    def test_withprompt(self, deforumanimkeys):
    ...
    @patch('deforum_helpers.parseq_adapter.DeforumAnimKeys')
    def test_withoutprompt(self, deforumanimkeys):
        parseq_args = SimpleNamespace(parseq_use_deltas=True, parseq_manifest=""" 
            {                
                "options": {
                    "output_fps": 30
    ...
    @patch('deforum_helpers.parseq_adapter.DeforumAnimKeys')
    def test_usedelta(self, deforumanimkeys):
        parseq_args = SimpleNamespace(parseq_use_deltas=True, parseq_manifest=""" 
            {                
                "options": {
                    "output_fps": 30
    ...
    @patch('deforum_helpers.parseq_adapter.DeforumAnimKeys')
    def test_usenondelta(self, deforumanimkeys):
    ...
    @patch('deforum_helpers.parseq_adapter.DeforumAnimKeys')
    def test_fallbackonundefined(self, deforumanimkeys):
        parseq_args = SimpleNamespace(parseq_use_deltas=False, parseq_manifest=""" 
            {                
                "options": {
                    "output_fps": 30
